<script lang="ts">
  ...
  async function regenerateStorage() { try { await requestJson('/api/pages/storage/regenerate', 'POST', {}); await loadPages(); templatesOpen = false; notice = 'Storage pages regenerated from inventory'; } catch (cause) { error = String(cause); } }
  ...
  {#if active === 'Pages'}
    <section class="pages-layout">
      ...
      <section class="workspace card">
        ...
        {#if templatesOpen}<div class="surface"><button class="surface-close" aria-label="Close templates" onclick={() => templatesOpen = false}>×</button><TemplatePicker oncreate={createPage} onstorage={regenerateStorage} /></div>{/if}
