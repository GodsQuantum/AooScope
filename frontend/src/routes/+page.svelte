<script lang="ts">
  ...
  async function createPage() { const selectionGeneration = pageRequestGeneration; try { const created = await requestJson<Page>('/api/pages', 'POST', { name: 'New page' }); await refreshPagesPreservingCarousel(); if (selectionGeneration === pageRequestGeneration) await loadPage(created.id); } catch (cause) { error = String(cause); } }
