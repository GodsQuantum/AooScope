<script lang="ts">
  ...
  import CarouselSidebar, { type PageSummary } from '$lib/pages/CarouselSidebar.svelte';
  ...
  async function createPage() { try { const created = await requestJson<Page>('/api/pages', 'POST', { name: 'New page' }); await refreshPagesPreservingCarousel(); await loadPage(created.id); } catch (cause) { error = String(cause); } }
  async function duplicatePage(id: string) { try { const created = await requestJson<Page>(`/api/pages/${id}/duplicate`, 'POST', {}); await refreshPagesPreservingCarousel(); await loadPage(created.id); } catch (cause) { error = String(cause); } }
  async function restorePage(id: string) { if (!confirm('Restore this factory page? Your page customizations will be replaced.')) return; try { await requestJson<Page>(`/api/pages/${id}/restore`, 'POST', {}); await refreshPagesPreservingCarousel(); await loadPage(id); notice = 'Factory page restored'; } catch (cause) { error = String(cause); } }
  async function deletePage(id: string) { if (!confirm('Delete this page?')) return; try { await requestJson(`/api/pages/${id}`, 'DELETE'); await refreshPagesPreservingCarousel(); if (page?.id === id) { const next = pages[0]; if (next) await loadPage(next.id); else page = undefined; } notice = 'Page deleted'; } catch (cause) { error = String(cause); } }
...
<main>
  ...
  {#if active === 'Pages'}
    <section class="pages-layout">
      <CarouselSidebar {pages} selected={page?.id} onselect={selectPage} onchange={(next) => pages = next} onduplicate={duplicatePage} onrestore={restorePage} ondelete={deletePage} onsave={saveCarousel} oncreate={createPage} />
      <section class="workspace card">
        <div class="workspace-head"><div><span class="eyebrow">Visual designer</span><h2>{page?.name ?? 'Select a page'}</h2><p>Logical canvas · 960 × 376</p></div><div class="toolbar"><button onclick={savePage}>Save page</button><button onclick={preview}>Preview</button><button class="primary" onclick={apply}>Apply to LCD</button></div></div>
        <div class="designer"><aside class="palette"><WidgetPalette onadd={addWidget} {metrics} {assets} /></aside><div class="canvas-shell"><Canvas layers={page?.layers ?? []} background={page?.background?.color} {selected} onselect={(id) => selected = id} onchange={updateLayer} /></div><aside class="inspector"><Inspector layer={page?.layers.find((layer) => layer.id === selected)} {metrics} onchange={(changes) => selected && updateLayer(selected, changes)} /></aside></div>
...
<style>
  :global(*){box-sizing:border-box}:global(html){background:#061019}:global(body){margin:0;min-width:0;background:radial-gradient(circle at 48% -15%,#12334a 0,#081622 38%,#050d15 78%);color:#edf7fc;font-family:Inter,ui-sans-serif,system-ui,sans-serif;min-height:100vh}:global(button),:global(input),:global(select){font:inherit}:global(button){border:1px solid #315267;border-radius:8px;padding:7px 10px;background:#102533;color:#dfedf4;cursor:pointer}:global(button:hover){filter:brightness(1.15)}:global(button:disabled){cursor:not-allowed;opacity:.4}:global(input),:global(select){min-width:0;padding:8px 9px;border:1px solid #304f63;border-radius:8px;background:#07141e;color:#edf7fc}:global(label){color:#91a8b8;font-size:.75rem}main{width:min(1500px,100%);margin:auto;padding:22px 24px 60px;overflow:clip}header{display:flex;justify-content:space-between;align-items:center;gap:20px}.brand{display:flex;align-items:center;gap:13px}.brand>i{width:43px;height:43px;border:2px solid #35d9ff;border-radius:50%;box-shadow:0 0 22px #35d9ff33,inset 0 0 18px #35d9ff22;position:relative}.brand>i:after{content:'';position:absolute;width:8px;height:8px;left:16px;top:-5px;border-radius:50%;background:#58e5a4;box-shadow:0 0 10px #58e5a4}.brand span,.eyebrow{color:#35d9ff;font-size:.65rem;font-weight:800;letter-spacing:.16em}.brand h1{margin:1px 0;font-size:2rem;line-height:1}.brand p{margin:0;color:#758e9e;font-size:.72rem;letter-spacing:.12em;text-transform:uppercase}.alert{display:flex;justify-content:space-between;gap:12px;margin-top:14px;padding:10px 12px;border:1px solid #713b46;border-radius:10px;background:#2a151b;color:#ff9ba6}.alert button{border:0;background:none;padding:0}.notice{margin:10px 0 -2px;color:#58e5a4;font-size:.75rem}.card,.panel{border:1px solid #263f52;border-radius:18px;background:linear-gradient(145deg,#0e1e2bfa,#0a1621fa);box-shadow:0 20px 50px #0004,inset 0 1px #ffffff09}.pages-layout{display:grid;grid-template-columns:290px minmax(0,1fr);gap:16px;margin-top:18px}.card{padding:16px}.workspace{min-width:0}.workspace-head{display:flex;justify-content:space-between;align-items:end;gap:16px}.workspace-head h2{margin:4px 0}.workspace-head p{margin:0;color:#839baa;font-size:.75rem}.toolbar{display:flex;gap:7px;flex-wrap:wrap}.primary{background:#087fa5;border-color:#35d9ff;color:white}.designer{display:grid;grid-template-columns:178px minmax(0,1fr) 220px;gap:12px;margin-top:15px}.palette,.inspector,.canvas-shell{min-width:0;border:1px solid #213b4c;border-radius:13px;background:#07131d;padding:11px}.palette,.inspector{max-height:550px;overflow:auto}.canvas-shell{align-self:start;background:#030a10}.preview{display:grid;gap:8px;margin-top:14px;padding-top:14px;border-top:1px solid #263f52}.preview div{display:flex;justify-content:space-between}.preview h2{margin:0}.preview img{width:100%;height:auto;border-radius:10px}.panel{margin-top:18px;padding:18px}@media(max-width:1120px){.pages-layout{grid-template-columns:1fr}.designer{grid-template-columns:160px minmax(0,1fr)}.inspector{grid-column:1/-1}}@media(max-width:760px){main{padding:15px 12px 45px}header{align-items:flex-start}.pages-layout{margin-top:12px}.workspace-head{align-items:flex-start;flex-direction:column}.toolbar{width:100%}.toolbar button{flex:1}.designer{grid-template-columns:1fr}.canvas-shell{order:-1;padding:7px}.palette,.inspector{max-height:none}.inspector{grid-column:auto}.panel{padding:13px}}@media(max-width:480px){header{flex-direction:column}.brand h1{font-size:1.7rem}.toolbar button{flex-basis:40%}}
</style>
