<script lang="ts">
  ...
  let clearRequested = $state<Record<string, boolean>>({});
  let message = $state('');
  ...
  function clearCredentials(id: string) { clearRequested[id] = true; }
  async function save() {
    message = 'Saving…';
    try {
      const clean = Object.fromEntries(Object.entries(draft).map(([id, value]) => {
        const config = Object.fromEntries(Object.entries(value).filter(([key, field]) => key !== 'secret_set' && field !== ''));
        if (clearRequested[id]) for (const field of catalog.find((item) => item.id === id)?.credential_fields ?? []) config[field] = null;
        return [id, config];
      }));
      const existing = settingsDocument?.providers && typeof settingsDocument.providers === 'object' ? settingsDocument.providers as Record<string, unknown> : settings;
      const document = await requestJson<Record<string, unknown>>('/api/settings', 'PUT', { ...(settingsDocument ?? {}), providers: { ...existing, ...clean } });
      message = 'Providers saved'; onsaved?.(document);
      for (const item of catalog) { for (const field of item.credential_fields) if (draft[item.id]) draft[item.id][field] = ''; if (clearRequested[item.id]) draft[item.id].secret_set = false; }
      clearRequested = {};
      return true;
    ...
    {#each catalog as item (item.id)}
      ...
      {#if config}<article class="provider-card">
        ...
        {#if item.id !== 'local'}
          ...
          <div class="secret"><span class:present={config.secret_set && !clearRequested[item.id]}>{clearRequested[item.id] ? 'Credential will be cleared' : config.secret_set ? 'Credential saved' : 'No credential saved'}</span><label class="check"><input type="checkbox" bind:checked={config.verify_tls} />Verify TLS</label></div>
          {#if config.secret_set && item.credential_fields.length}<button class="clear" aria-label={`Clear stored ${item.name} credentials`} onclick={() => clearCredentials(item.id)}>Clear stored credentials</button>{/if}
