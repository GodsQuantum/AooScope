<script lang="ts">
  ...
  let { event = {}, presets = [], onorbit, onpreview }: { event?: Event; presets?: Preset[]; onorbit: () => void; onpreview: () => void } = $props();
  const demo: Event = { mode: 'playing', title: 'Cloud 9 — Offline demo', progress_pct: 64, provider_chain: ['Jellyfin · demo'] };
