...
class AooScopeWebTests(unittest.TestCase):
    ...
    def test_all_jquery_id_selectors_exist_in_html(self):
        import re
        response = self.client.get("/")
        text = response.get_data(as_text=True)
        ids = set(re.findall(r"id=[\"\']([^\"\']+)[\"\']", text))
        refs = set(re.findall(r"\$\(\'[[]?#?", "")) if False else set(re.findall(r"\$\(\'#([^\']+)\'\)", text))
        missing = sorted(refs - ids)
        self.assertEqual(missing, [], f"JS references missing DOM ids: {missing}")

    def test_admin_disables_browser_cache(self):
        response = self.client.get("/")
